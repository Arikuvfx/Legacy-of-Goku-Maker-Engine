...
class BurningAttack(Projectile):
    ...
    def __init__(self, x, y, direction, stun_duration=1.5):
        super().__init__(x, y, direction)
        self.stun_duration = stun_duration

        # Projectile.__init__ already loaded ki_blast.png into self.frames.
        # Reload with the burning attack's own sheet — the same charge
        # sprite used by BurningChargeEffect doubles as the traveling
        # projectile — using the same frame_width/frame_height slicing so
        ...
        try:
            spritesheet = pygame.image.load(
                'assets/sprites/attacks/burning_attack/charge_burning_attack.png'
            ).convert_alpha()
...
class BurningHitEffect:
    ...
    def __init__(self, x, y):
        ...
        try:
            spritesheet = pygame.image.load(
                'assets/sprites/attacks/burning_attack/hit_burning_attack.png'
            ).convert_alpha()
            sheet_width = spritesheet.get_width()
            num_frames = sheet_width // self.frame_width
            for i in range(num_frames):
